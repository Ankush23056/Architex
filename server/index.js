...
io.on("connection", async (socket) => {
  const getUniqueUsername = () => {
    ...
    let name = base,
      count = 1;
    const taken = Array.from(io.sockets.sockets.values()).map(
      (s) => s.username,
    );
