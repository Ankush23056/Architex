...
function computeGuides(movingEl, allElements, movingIds) {
  ...
  allElements
    .filter((el) => !movingIds.includes(el.id))
    .forEach((el) => {
      let ex = el.x,
        ey = el.y,
        ew = el.width || 0,
      ...
      cEdgesX.forEach((cx) => {
        edgesX.forEach((mx2) => {
          if (Math.abs(mx2 - cx) <= ALIGN_THRESHOLD) {
            if (
              !guides.some((g) => g.type === "v" && Math.abs(g.value - cx) < 1)
            )
      ...
      cEdgesY.forEach((cy) => {
        edgesY.forEach((my2) => {
          if (Math.abs(my2 - cy) <= ALIGN_THRESHOLD) {
            if (
              !guides.some((g) => g.type === "h" && Math.abs(g.value - cy) < 1)
            )
...
function drawGuides(ctx, guides, width, height) {
  ...
  guides.forEach((g) => {
    ...
    if (g.type === "v") {
      ...
      ctx.lineTo(g.value, height);
    } else {
      ctx.moveTo(0, g.value);
      ctx.lineTo(width, g.value);
    }
    ctx.stroke();
  });
  ctx.setLineDash([]);
    ...
    const render = () => {
      ...
      sorted.forEach((el) => {
        ...
        if (isDragging.current && startOffsets.current[el.id]) {
          ...
          if (el.type === "path" && off.points) {
            const sdx = sx - off.x,
              sdy = sy - off.y;
            renderEl.points = off.points.map((p) => ({
              x: p.x + sdx,
              y: p.y + sdy,
            }));
          }
        ...
        if (renderEl.id === selId || selIds.includes(renderEl.id)) {
          if (renderEl.type === "curve" && renderEl.id === selId) {
            ...
            if (renderEl.type === "path" && renderEl.points?.length > 0) {
              ...
              const bx = Math.min(
                renderEl.x,
                renderEl.x + (renderEl.width || 0),
              );
              const by = Math.min(
                renderEl.y,
                renderEl.y + (renderEl.height || 0),
              );
              ctx.strokeRect(
                bx - pad,
                by - pad,
                Math.abs(renderEl.width || 0) + pad * 2,
                Math.abs(renderEl.height || 0) + pad * 2,
              );
            }
            ctx.restore();
          }
      ...
      if (selIds.length > 1) {
        ...
        selIds.forEach((id) => {
          ...
          const ex = el.x,
            ey = el.y;
          const ew =
            el.type === "path" && el.points?.length
              ? Math.max(...el.points.map((p) => p.x)) -
                Math.min(...el.points.map((p) => p.x))
              : el.width || 0;
          const eh =
            el.type === "path" && el.points?.length
              ? Math.max(...el.points.map((p) => p.y)) -
                Math.min(...el.points.map((p) => p.y))
              : el.height || 0;
      ...
      if (mq) {
        ...
        ctx.fillRect(
          mq.startX,
          mq.startY,
          mq.currentX - mq.startX,
          mq.currentY - mq.startY,
        );
        ctx.strokeRect(
          mq.startX,
          mq.startY,
  ...
  const handlePointerDown = (e) => {
    ...
    if (currentToolRef.current === "select") {
      ...
      if (clicked) {
        ...
        const group = selectedIdsRef.current.includes(clicked.id)
      ...
      const newEl = {
        ...
        zIndex:
          elementsRef.current.length > 0
            ? Math.max(...elementsRef.current.map((e) => e.zIndex)) + 1
            : 0,
  ...
  const handlePointerMove = (e) => {
    ...
    if (currentToolRef.current === "select" && isDragging.current) {
      ...
      if (currentToolRef.current === "pen") {
        const last =
          currentElement.current.points[
            currentElement.current.points.length - 1
          ];
  ...
  const handlePointerUp = () => {
    ...
    if (mq) {
      ...
      const minX = Math.min(mq.startX, mq.currentX),
        maxX = Math.max(mq.startX, mq.currentX);
      const minY = Math.min(mq.startY, mq.currentY),
        maxY = Math.max(mq.startY, mq.currentY);
      ...
      const hit = elementsRef.current.filter((el) => {
        ...
        return (
          Math.min(ex, ex + ew) >= minX &&
          Math.max(ex, ex + ew) <= maxX &&
          Math.min(ey, ey + eh) >= minY &&
          Math.max(ey, ey + eh) <= maxY
        );
      });
    ...
    if (isDragging.current) {
      ...
      if (updated.length > 0) {
        updateElementsBulk && updated.length > 1
    ...
    () =>
      Object.defineProperties(
        {},
        {
          ...
          currentColor: {
            get: () => currentColorRef.current,
            enumerable: true,
          },
          currentStrokeWidth: {
            get: () => currentStrokeWidthRef.current,
            enumerable: true,
          },
          // Parent notification setters
          _onSelectedIdChange: {
            set: (fn) => {
              onSelectedIdChange.current = fn;
            },
          },
          _onSelectedIdsChange: {
            set: (fn) => {
              onSelectedIdsChange.current = fn;
            },
          },
          _onToolChange: {
            set: (fn) => {
              onToolChange.current = fn;
            },
          },
          _onColorChange: {
            set: (fn) => {
              onColorChange.current = fn;
            },
          },
          _onStrokeChange: {
            set: (fn) => {
              onStrokeChange.current = fn;
            },
          },
        },
      ),
