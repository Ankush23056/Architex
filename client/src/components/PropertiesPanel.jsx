...
export function PropertiesPanel({
  ...
  const scrambleText = useScramble(
    isAnalyzing ? "SCANNING..." : "AI ANALYZE",
    isAnalyzing,
  );
  const API_BASE =
  ...
  const onHeaderMouseDown = useCallback(
    (e) => {
      ...
      const onMove = (me) => {
        ...
        const nx =
          dragRef.current.originX + (me.clientX - dragRef.current.startX);
        const ny =
          dragRef.current.originY + (me.clientY - dragRef.current.startY);
        ...
        >
              ...
              {colors.map((c) => (
                <button
                  ...
                  style={{
                    backgroundColor: c,
                    borderColor:
                      (selectedElement?.color || currentColor) === c
                        ? "#fff"
                        : "#333",
                    boxShadow:
                      (selectedElement?.color || currentColor) === c
          ...
          {aiResult && (
            ...
            >
              ...
              <div className="p-3">
                <div className="text-groq-orange font-bold uppercase border-b border-groq-orange/30 pb-1 mb-3">
                  AI Analysis
                </div>
                ...
                {aiResult.analysis && aiResult.analysis.length > 0 && (
                  <ul className="list-disc pl-4 space-y-2 opacity-90">
                    {aiResult.analysis.map((point, idx) => (
                      <li key={idx}>
                        <TypewriterText text={point} delay={15} />
                      </li>
                    ))}
                  </ul>
                )}
                ...
                {aiResult.missing && aiResult.missing.length > 0 && (
                  <div className="mt-4">
                    <div className="uppercase opacity-70 mb-2 mt-2 text-groq-orange">
